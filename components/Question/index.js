    ...
    <>
      {data && (
        <div
          ...
          data-id={data[currentQuestionIndex].id}
        >
          ...
          <div className='grid grid-columns-1 grid-rows-4 gap-y-4'>
            {data[currentQuestionIndex].answers.map((answer) => {
              return (
                <p
                  className='flex justify-center items-center text-justify p-5 cursor-pointer	border-2 rounded-lg hover:border-secondary'
                  key={answer.id}
